# AI Agent Context — mdburst.com

## Summary

*one paragraph: what this site is, what it does*

mdburst.com helps markdown writers stop wasting time reformatting the same draft for multiple channels. A creator writes once, then mdburst converts that file into destination-specific outputs for blogs, newsletters, LinkedIn, and social threads, preserving links, headings, spacing, embeds, and platform-safe formatting.

## Audience

*one sentence: who this is for (broad demographic)*

Markdown-first creators, indie bloggers, technical founders, devrel writers, newsletter operators, and agencies publishing the same ideas across multiple channels.
...
The strongest ICP is a weekly publisher who already writes in markdown and distributes every post to at least three places: a blog, a newsletter, and one or more social platforms. They feel the pain every week, value workflow speed, dislike generic AI writing tools, and will pay for formatting reliability, preview accuracy, and one-click publishing more than content generation.

## Goals

*1-2 sentences: primary business / product goal*

Validate whether creators will pay to eliminate manual reformatting and multi-channel publishing busywork. The first product goal is to convert markdown into Ghost- and Buffer-ready outputs with reliable previews.
...
`proj=mdburst.com`, so these all work either from this dir or from `sites/`:

| Command | What it does |
|---|---|
| `make buildsh` *(from `sites/`)* | Drop into a bash shell inside the docker container at `/usr/src/app` (= `sites/` mounted in). |
| `make run` *(from here)* / `make run proj=mdburst.com` *(from `sites/`)* | `pnpm install` then start dev server (auto-detected). |
| `make check-vite proj=mdburst.com` | Start the dev server, skipping install. |
| `make test proj=mdburst.com` | `pnpm install` + `pnpm build` + `pnpm test`. **Hard-fails outside docker** — `make buildsh` first, or `docker exec`. |
...
docker exec -w /usr/src/app <name> make test proj=mdburst.com
```
...
- **Deploy:** `wrangler deploy` (locally) or via Cloudflare's Git integration on push.
  ...
  `cd ../portfolio && make run ARGS="deploy mdburst.com"` runs `gh repo create` and
...
Own the “markdown publishing workflow” niche with practical content: markdown-to-Ghost guides, markdown newsletter formatting, LinkedIn formatting fixes, social thread splitting, Obsidian-to-blog workflows, and comparisons against manual publishing. Publish templates, checklists, and free converters as SEO entry points.

### Post-deploy checklist (do these once after the first successful deploy)

- [ ] Verify in **Google Search Console** at https://search.google.com/search-console — add as `sc-domain:mdburst.com` property; verify via DNS TXT record. Until this is done, no SEO traffic data is observable for this site (and the workspace-wide `30 commercial sites with traffic` goal can't credit it).
- [ ] Submit the sitemap (`https://mdburst.com/sitemap.xml`) inside GSC.
- [ ] Update the **Live URL** above with the actual deploy URL.
- [ ] Run `make run ARGS="cleanup"` from `sites/portfolio/` so `data/portfolio.json` reflects the new project's state (and `project status mdburst.com` resolves cleanly).
...
`portfolio project check mdburst.com` (run from `sites/portfolio/`).
